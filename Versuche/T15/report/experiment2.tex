...
\end{equation*}
\noindent
The predicted Room temperature does fit the expected value. Still, we have a $\chi^2=4.9$. Looking at the plot, the reason seems to be of systematic natur. As a reason, we suspect that the cooling of the room temperature by opening the windows has an effect on the measurement.\\
A more fitting modell considers this behavior by adding a linear Term:
\begin{equation}
    T(t) = (T_0-T_R)e^{-kt}-T_R + at
\end{equation}
\noindent
...
\begin{equation*}
    \begin{aligned}
        T_R &= (34.91\pm 9.66\cdot 10^{-4}) \unit{\celsius}\\
        T_0 &= (76.25\pm 2.31\cdot 10^{-4})\unit{\celsius}\\
        k &=(7.61\cdot 10^{-4} \pm 1.04\cdot 10^{-12}) \unit{\second^{-1}}
        a &= =(1.577\cdot 10^{-3} \pm 2.36\cdot 10^{-11}) \unit{\second^{-1}}
    \end{aligned}
\end{equation*}
\noindent
